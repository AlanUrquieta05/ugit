...
def parse_args():
    ...
    #Creating a destination to store subcommands under the command attribute
    commands = parser.add_subparsers (dest='command')
    commands.required = True

    oid = base.get_oid

    #Creates a subcommand for init
    init_parser = commands.add_parser('init')
    init_parser.set_defaults(func = init)

    #Creates a subcommand for hash-object
    hash_object_parser = commands.add_parser('hash-object')
    hash_object_parser.set_defaults(func = hash_object)
    #Allows a file to be passed into argumet for hash
...
def init(args):
    base.init()
    # os.getcdw gets the current working directery i.e. "user/alan/ugit"
    print(f'Initialized empty ugit repository in {os.getcwd()}/{data.GIT_DIR}')

#This will take a file and store it in the .ugit directory
def hash_object(args):
    with open(args.file, 'rb') as f:
        print (data.hash_object (f.read()))

# Will dislpay all the data in a file using its OID
def cat_file(args):
    sys.stdout.flush()
...
def _diff(args):
    ...
    if args.commit:
        tree_from = base.get_tree(oid and base.get_commit(oid).tree1)
    if args.cached:
...
def merge_base(args):
    print(base.get_merged_base(args.commit1, args.commit2))
