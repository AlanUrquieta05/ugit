...
def checkout(name):
    ...
    if is_branch(name):
        HEAD = data.RefValue(symbolic=True,value=f'ref/heads/{name}')
    else:
...
def merge(other):
    HEAD = data.get_ref('HEAD').value
    assert HEAD
    merge_base = get_merged_base(other, HEAD)
...
def get_oid(name):
    ...
    # name is SHA1, checking if name is an OID if not a tag
